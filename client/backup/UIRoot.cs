...
public class UIRoot : MonoBehaviour
{
	...
	/// <summary>
	/// 在shrinkPortraitUI模式下如果height/width>heightWidthRatio将以宽度拉伸.
	/// </summary>
	public float baseHeight = 640;
	...
	public int activeHeight
	{
		get
		{
			...
			if (scalingStyle == Scaling.FixedSizeOnMobiles)
				return manualHeight;
#endif
			if (scalingStyle != Scaling.Custom)
			{
			...
			if (shrinkPortraitUI)
			{
				if (scalingStyle == Scaling.Custom)
				{
					if (mode == Orientation.Vertical)
					{
						...
						{
							height = Mathf.RoundToInt(baseWidth / (float)w * (float)h);//基于宽度
						}
						else
						{
							height = Mathf.RoundToInt(baseHeight);
						}
					}
					else if (mode == Orientation.Horizontal)//NGUI原来是基于高度适配的，所以只需返回baseHeight
					{
                        height = Mathf.RoundToInt(baseHeight);
                    }
				}
				...
				{
					height = Mathf.RoundToInt(height * ((float)h / w));
				}
			}
