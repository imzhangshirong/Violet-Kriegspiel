...
static public class NGUIRichText
{
    ...
    static public UIAtlas emotionAtlas;
    static public List<RichTextEmotionItem> richTextEmotions;
    static public List<RichTextPrefabItem> richTextPrefabs;
    static public int OfontSize = 16;
	...
	static public void Update (bool request)
	{
		...
		useSymbols = (dynamicFont != null || bitmapFont != null || richTextEmotions!=null || richTextPrefabs!=null) && encoding && symbolStyle != SymbolStyle.None;
	...
	{
        BMSymbol symbol = (bitmapFont != null) ? bitmapFont.MatchSymbol(text, index, textLength) : null;
        if (symbol == null && richTextEmotions != null)
        {
            symbol = MatchEmotionSymbol(text, index, textLength);
        }

        if (symbol == null && richTextPrefabs != null)
        {
            symbol = MatchPrefabSymbol(text, index, textLength);
        }
        return symbol;
	}


    private static BMSymbol MatchEmotionSymbol(string text, int offset, int textLength)
    {
        // No symbols present
        float scale = (float)fontSize / OfontSize;
        int count = richTextEmotions.Count;
        ...
        {
            RichTextEmotionItem item = richTextEmotions[i];
            // If the symbol's length is longer, move on
            int symbolLength = item.sequence.Length;
            ...
            {
                if (text[offset + c] != item.sequence[c])
                {
                    match = false;
                    break;
                }
            ...
            if (match)
            {
                ...
                sym.SetEmotionItem(emotionAtlas,item,scale);
                return sym;
            }
        }
        return null;
    ...
    private static BMSymbol MatchPrefabSymbol(string text, int offset, int textLength)
    {
        // No symbols present
        float scale = (float)fontSize / OfontSize;
        int count = richTextPrefabs.Count;
        if (count == 0) return null;
        textLength -= offset;
        ...
        for (int i = 0; i < count; ++i)
        {
            RichTextPrefabItem item = richTextPrefabs[i];
            // If the symbol's length is longer, move on
            int symbolLength = item.sequence.Length;
            if (symbolLength == 0 || textLength < symbolLength) continue;
	...
	/// <summary>
	/// Align the vertices to be right or center-aligned given the line width specified by NGUIRichText.lineWidth.
	/// </summary>

	static public void Align (List<Vector3> verts, int indexOffset, float printedWidth, int elements = 4, int richItemIndex = 0,List<RichTextItem> richItems = null,float additionHeight = 0f)
	{
		switch (alignment)
		{
			case Alignment.Right:
			{
                ...
                if (richItems != null)
                {
                    for (int i = richItemIndex, imax = richItems.Count; i < imax; ++i)
                    {
                        richItems[i].area.x += padding;
                        richItems[i].area.y += additionHeight;
                    }
                }
				break;
			}
		...
		if (!string.IsNullOrEmpty(text))
		{
			...
			for (int i = 0; i < textLength; ++i)
			{
				...
				if (symbol != null)
				{
                    ...
                    {
                        if (mx > maxX) maxX = mx;
                        float d = symbol.height - fontSize;
                        if (d > additionHeight) additionHeight = d;
                    }
			...
			v.y = Mathf.Ceil((y + finalLineHeight + additionHeight));
		}
		return v;
	}
		...
		for (; offset < textLength; ++offset)
		{
			...
			if (ch == '\n')
			{
				...
				if (wrapLineColors)
				{
					...
					{
						sb.Append("[");
						sb.Append(NGUIRichText.EncodeColor(mColors[i]));
						sb.Append("]");
					}
				}
	...
	static public void Print (string text, List<Vector3> verts, List<Vector2> uvs, List<Color> cols, List<RichTextItem> richItems = null)
	{
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (ch == '\n')
			{
                ...
                Align(verts, indexOffset, x - finalSpacingX, 4, richItemIndex, richItems, additionHeight);
                y += additionHeight;
                additionHeight = 0f;
                if (richItems != null) richItemIndex = richItems.Count;
			...
			if (symbol != null)
			{
                ...
                if (richItems != null)
                {
                    ...
                    item.area = new Rect(symbol.advance + x, y, v1x - v0x, v1y - v0y);
                    item.spriteName = symbol.spriteName;
                    item.sequence = symbol.sequence;
                    item.scale = scale;
                    item.baseLine = baseline;
                    item.textLineHeight = finalLineHeight;
                ...
                if ((richTextPrefabs != null && richTextPrefabs.Count > 0) || (richTextEmotions != null && richTextEmotions.Count > 0)) continue;//do not draw
	...
	static public void PrintApproximateCharacterPositions (string text, List<Vector3> verts, List<int> indices)
	{
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (ch == '\n')
			{
                Align(verts, indexOffset, x - finalSpacingX, 1, 0, null, additionHeight);
                y += additionHeight;
                additionHeight = 0f;
                indexOffset = verts.Count;
	...
	static public void PrintExactCharacterPositions (string text, List<Vector3> verts, List<int> indices)
	{
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (ch == '\n')
			{
                ...
                Align(verts, indexOffset, x - finalSpacingX, 4, 0, null, additionHeight);
                y += additionHeight;
                additionHeight = 0f;
                indexOffset = verts.Count;
			...
			if (symbol == null)
			{
				...
				if (gw != 0f)
				{
					...
					if (x + w > maxWidth)
					{
						...
						if (indexOffset < verts.Count)
						{
                            Align(verts, indexOffset, x - finalSpacingX, 2, 0, null, additionHeight);
                            y += additionHeight;
                            additionHeight = 0f;
                            indexOffset = verts.Count;
						}
	...
	static public void PrintCaretAndSelection (string text, int start, int end, List<Vector3> caret, List<Vector3> highlight)
	{
		...
		for (; index < textLength; ++index)
		{
			...
			if (ch == '\n')
			{
				...
				{
                    Align(caret, caretOffset, x - finalSpacingX, 4, 0, null, additionHeight);
                    y += additionHeight;
                    additionHeight = 0f;
                    caret = null;
				}
				...
				if (highlight != null)
				{
					...
					// Align the highlight
					if (highlightOffset < highlight.Count)
					{
                        Align(highlight, highlightOffset, x - finalSpacingX, 4, 0, null, additionHeight);
                        y += additionHeight;
                        additionHeight = 0f;
		...
		if (highlight != null)
		{
			...
			if (highlightOffset < highlight.Count)
            {
                Align(highlight, highlightOffset, x - finalSpacingX, 1, 0, null, additionHeight);
                y += additionHeight;
                additionHeight = 0f;
            }
		}
