...
[ExecuteInEditMode]
[AddComponentMenu("NGUI/UI/NGUI RichTextLabel")]
public class UIRichTextLabel : UIWidget
{
	public enum Effect
	{
		None,
    ...
    [HideInInspector][SerializeField]List<RichTextPrefabItem> mPrefabs = new List<RichTextPrefabItem>();
    public List<RichTextPrefabItem> Prefabs { get { return mPrefabs; } set { mPrefabs = value; } }

    [HideInInspector][SerializeField]List<RichTextEmotionItem> mEmotions = new List<RichTextEmotionItem>();
    public List<RichTextEmotionItem> Emotions { get { return mEmotions; } set { mEmotions = value; } }

    [HideInInspector][SerializeField]UIAtlas mEmotionAtlas;
    public UIAtlas EmotionAtlas { get { return mEmotionAtlas; } set { mEmotionAtlas = value; } }
	...
	[Obsolete("Use UIRichTextLabel.bitmapFont instead")]
	...
	[System.Obsolete("Use 'overflowMethod == UIRichTextLabel.Overflow.ShrinkContent' instead")]
	...
	static BetterList<UIRichTextLabel> mList = new BetterList<UIRichTextLabel>();
	...
	static void OnFontChanged (Font font)
	{
		...
		for (int i = 0; i < mList.size; ++i)
		{
			...
			if (lbl != null)v [kp;' 5rfc328i0u
			{
				...
				if (fnt == font)
				{
					if (mTempLabels == null) mTempLabels = new BetterList<UIRichTextLabel>();

					if (!mTempLabels.Contains(lbl))
					{
						mTempLabels.Add(lbl);
						lbl.UpdateNGUIRichText();
	...
	//[System.NonSerialized] static BetterList<UIRichTextLabel> mTempLabels;
	...
	/// <summary>
	/// UIRichTextLabel needs additional processing when something changes.
	/// </summary>
	...
	public void ProcessText (bool legacyMode = false, bool full = true)
	{
		...
		if (mOverflow == Overflow.ResizeFreely)
		{
			...
			if (mOverflowWidth > 0)
			{
				NGUIRichText.rectWidth = Mathf.Min(NGUIRichText.rectWidth, mOverflowWidth);
				NGUIRichText.regionWidth = Mathf.Min(NGUIRichText.regionWidth, mOverflowWidth);
			}
		}
		...
		if (mFinalFontSize > 0)
		{
			...
			for (int ps = mFinalFontSize; ps > 0; --ps)
			{
				...
				// Wrap the text
				bool fits = NGUIRichText.WrapText(printedText, out mProcessedText, false, false, mOverflow == Overflow.ClampContent && mOverflowEllipsis);
	...
	[System.Obsolete("Use UIRichTextLabel.GetCharacterAtPosition instead")]
	...
	public override void OnFill (List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
	{
        for (int i = gameObject.transform.childCount - 1; i >= 0; i--)
        {
            ...
            if (panel.widgets.IndexOf(widget) > -1)
            {
        ...
        List<RichTextItem> richItems = new List<RichTextItem>();

		NGUIRichText.tint = col;
		NGUIRichText.Print(text, verts, uvs, cols, richItems);
		NGUIRichText.bitmapFont = null;
		NGUIRichText.dynamicFont = null;
        ...
        for (int i = 0; i < richItems.Count; i++)
        {
            ...
            switch (item.type)
            {
                ...
                case 1://表情
                    go = new GameObject();
                    UISprite itemSprite = go.AddComponent<UISprite>();
                    itemSprite.atlas = EmotionAtlas;
                    itemSprite.spriteName = item.spriteName;
                    itemSprite.name = item.spriteName;
                    itemSprite.MakePixelPerfect();
                    itemSprite.depth = depth + 1;
                    itemSprite.gameObject.layer = gameObject.layer;//强制同一层layer
                    RichTextEmotion emotion = go.AddComponent<RichTextEmotion>();
                    break;
                case 2://自定义Prefab
                    go = OnGetRichTextItem(item);
                    break;
            }
            if (go != null)
            {
                float fscale = (float)mFinalFontSize / mFontSize;
                Vector3 scale = go.transform.localScale;
                go.transform.parent = gameObject.transform;
                go.transform.localPosition = new Vector3(x, y, 0);
                go.transform.localScale = scale * fscale;
            }
	...
	/// <summary>
	/// Update NGUIRichText.current with all the properties from this label.
	/// </summary>

	public void UpdateNGUIRichText ()
	{
        ...
        NGUIRichText.emotionAtlas = EmotionAtlas;
        NGUIRichText.richTextPrefabs = Prefabs;
        NGUIRichText.richTextEmotions = Emotions;
        NGUIRichText.OfontSize = mFontSize;
    ...
    public virtual GameObject OnGetRichTextItem(RichTextItem ItemPositionData)
    {
        for (int i = 0; i < Prefabs.Count; i++)
        {
            if (Prefabs[i].prefabObject != null)
            {
                if (ItemPositionData.sequence == Prefabs[i].sequence)
                {
                    Debug.Log(Prefabs[i].prefabObject.name);
                    GameObject go = (GameObject)Instantiate(Prefabs[i].prefabObject);
                    go.name = Prefabs[i].prefabObject.name;
                    return go;
                }
            }
